...
Each node's edges are split by fixed-width ranges of persistent source IDs.
Ranges survive dense renumbering and additions. They are storage/update units,
...
class FragmentedSemanticHierarchyField(SemanticHierarchyField):
    def __init__(self,kernel,root,labels,pool=None,scope=None,max_inputs=32):
        ...
        buckets=lca[owner_index[sources],owner_index[targets]]; cohorts=tokens[sources]//max_inputs
        ...
        for index,node in enumerate(nodes):
            ...
            active_nodes+=1
            for cohort in np.unique(cohorts[node_edges]):
                edges=node_edges[cohorts[node_edges]==cohort]; key=(node.name,int(cohort))
        ...
        self.receipt=dict(prepare_s=time.perf_counter()-tick,hierarchy_nodes=len(nodes),active_hierarchy_nodes=active_nodes,
            ...
            max_inputs_per_fragment=max_inputs,partition='Fixed persistent input ID ranges inside each original hierarchy node')
