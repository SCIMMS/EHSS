"""Approximate numerical internal T with current entry anchors and foreign G.

Cached paths are rigidly posed and translated per row to start on the current
known atom. Internal endpoints/directions may differ from current owned sphere
reflection after strain: this is an explicit field-response approximation.
Physical atom coordinates are never snapped. Foreign arbitration uses current
geometry along the approximate internal path. A fresh-T control is required.
"""
...
@njit(cache=False)
def apply_strain_internal(rows, starts, origins, directions, targets, distances, atoms, rotation, translation,
    ...
    for i in range(n):
        ...
        # Anchor the complete cached polyline to the actual current entry.
        ...
        for segment in range(starts[i],starts[i+1]):
            if segment>starts[i]:
                p[i]=origins[segment]@rotation.T+translation+anchor
...
The shape gate controls approximation reuse, not a CCS error certificate.
Point/bond frame ambiguity inherits a common whole-model orientation. Source
points remain on current known atom surfaces; internal endpoints can drift.
"""
    def __init__(self, model, owners, source, cap=64, shape_tolerance=1e-10):
        if type(cap) is not int or cap<1: raise ValueError('Positive collision cap required')
        if not np.isfinite(shape_tolerance) or shape_tolerance<0:
            raise ValueError('Finite nonnegative shape tolerance required')
        ...
        self.cap=cap; self.shape_tolerance=float(shape_tolerance)
        self.frames=ReuseFrames(self.reference,np.zeros(len(self.reference),dtype=np.int64),self.owners)
    ...
    def bind(self,model,previous=None):
        ...
        a=self.reference-self.reference.mean(axis=0); b=world-world.mean(axis=0)
        if np.array_equal(world-world[0],self.reference-self.reference[0]): parent=np.eye(3)
        else:
            u,_,vh=np.linalg.svd(a.T@b); fix=np.eye(3)
            fix[2,2]=1. if np.linalg.det(u@vh)>=0 else -1.
            parent=(u@fix@vh).T
        ...
        for g,(atoms,rows) in enumerate(zip(self.atoms,self.rows)):
            b=world[atoms]; translation=b[0]
            rotation=self.frames.fit(g,world,parent[None,:,:])[0]
            ...
            if old is not None and residual<=self.shape_tolerance and np.array_equal(radii[atoms],old.radii):
            ...
            column=StrainColumnBlock(readonly(local.copy()),readonly(radii[atoms].copy()),
        ...
        receipt=dict(bind_s=time.perf_counter()-tick,reused_owners=reused,rebuilt_owners=rebuilt,
            ...
            strain_approximation=self.shape_tolerance>1e-8,shape_tolerance=self.shape_tolerance,
            approximate_reused_owners=[g for g in reused if residuals[g]>1e-8],
            material_gauge='Whole-model parent for point/bond roll; proper noncollinear fits',
            entry_anchor='Current known atom surface, translated stored polyline')
        return BoundStrainColumns(self,search,tuple(columns),tuple(rotations),tuple(translations),
            readonly(points),readonly(directions),readonly(tags),receipt)


@dataclass(frozen=True)
class BoundStrainColumns:
    bank: StrainColumns
    search: object
    columns: tuple
    rotations: tuple
    ...
    def apply(self):
        ...
        for g,(rows,column) in enumerate(zip(b.rows,self.columns)):
            ...
            result=apply_strain_internal(rows,column.starts,column.origins,column.directions,column.targets,column.distances,
        ...
        last_indices=history[np.arange(n),counts-1]
        s=self.search.model.spheres
        drift=np.abs(np.linalg.norm(points-s.centers[last_indices],axis=1)-s.radii[last_indices])
        active=terminal!=-3
        result=dict(points=points,directions=directions,counts=counts,history=history,terminal=terminal,
            ...
            weighted_endpoint_surface_drift=float(np.dot(b.weights[active],drift[active])),
            approximation='Entry-anchored stored internal polyline; exact current foreign spheres')
