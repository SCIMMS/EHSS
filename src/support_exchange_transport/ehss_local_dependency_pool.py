"""Empirical prefix reuse with recorded-flight support dependencies."""
...
class DependencyPrefixBatch(PrefixBatch):
    dependencies: FlightDependencies

class LocalDependencyPrefixPool(PrefixRefreshPool):
    """Old/current boxes gate recorded flights, not perturbed-path certificates."""
    def __init__(self,bank,geometry_tolerance=0.,point_tolerance=0.,direction_degrees=0.,max_age=4,reuse=True,padding=2.):
    ...
    def snapshot(self):
        p=LocalDependencyPrefixPool(self.bank,self.geometry_tolerance,self.point_tolerance,self.direction_degrees,self.max_age,self.reuse,self.padding)
    ...
    def trace(self,rows,model,source,keys,active,groups,rotations,bins,cap,allow_reentry=True):
        ...
        for batch,qi,ci in old_batches.values():
            ...
            if strict:
                ...
                motion=support_motion(local_world,batch.epoch.local_world,groups,len(self.bank.atoms))
                dep=batch.dependencies
                if np.any(motion>self.geometry_tolerance):
                    lo,hi=support_bounds(local_world,radii,groups,len(self.bank.atoms))
        ...
        for g in np.unique(owner[fresh]):
            ...
            dependencies=record_flights(source.origins[q],source.directions[q],history[q],counts[q],ports[q],world,radii,r,center,
                groups,len(self.bank.atoms),self.padding)
