"""Current physical coupling with exact selective material-state aggregation.

The audited aligned assembly is retained except for optional row aggregation.
Without a separate prefix pool all physical suffixes are still retraced.
"""
import time
import numpy as np
...
class SelectiveMaterialCollisionField(MaterialCollisionField):
    def __init__(self,bound,observed,physical_model,bins=2,checked_observations=False,allow_reentry=True,layout=None,vector_validation=True,prefix_pool=None,coupling_pool=None):
        ...
        if coupling_pool is None:
            ...
            starts,targets,prob,escape,residual=coupling_pool.assemble(layout,valid,atoms,target,observed['terminal'],bound.directions,mass,missing)
        ...
        self.receipt['selective_coupling']=None if coupling_pool is None else coupling_pool.receipt.copy()
        self.receipt['coupling_rebuilt']=coupling_pool is None or coupling_pool.receipt['reused_ports']==0
        self.receipt['physical_suffixes_recomputed']=prefix_pool is None
